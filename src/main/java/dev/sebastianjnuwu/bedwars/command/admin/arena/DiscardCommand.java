...
public class DiscardCommand extends BaseCommand implements SubCommand {
    ...
    public void execute(final CommandSender sender, final @NotNull String @NotNull [] args) {
        if (args.length < 2) {
            sender.sendMessage(this.lang.text(NamedTextColor.RED, "discard.usage"));
            return;
        }
        if (!(sender instanceof final Player player)) {
            sender.sendMessage(this.lang.text(NamedTextColor.RED, "discard.only_player"));
            return;
        }
        final String name = args[1];
        final Arena arena = this.arenaManager.get(name);
        if (arena == null) {
            sender.sendMessage(this.lang.text(NamedTextColor.RED, "discard.not_found", name));
            return;
        }
        if (!this.editorManager.isEditing(player, name)) {
            sender.sendMessage(this.lang.text(NamedTextColor.RED, "discard.not_editing", name));
            return;
        }
        ...
        sender.sendMessage(this.lang.text(NamedTextColor.GREEN, "discard.success", name));
    }

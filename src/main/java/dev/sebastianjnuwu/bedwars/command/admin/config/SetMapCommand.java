...
public class SetMapCommand extends BaseCommand implements ArenaSubCommand {
    ...
    public void execute(final CommandSender sender, final @NotNull Arena arena, final @NotNull String @NotNull [] args) {
        ...
        if (args.length < 4) {
            player.sendMessage(this.lang.text(NamedTextColor.RED, "admin.arena.setmap_usage"));
            return;
        }
        ...
        if ("default".equalsIgnoreCase(mapName)) {
            ...
            player.sendMessage(this.lang.text(NamedTextColor.GREEN, "admin.arena.setmap_reset", arena.getName()));
            return;
        }
        if (this.arenaManager.getMapFile(mapName) == null) {
            player.sendMessage(this.lang.text(NamedTextColor.RED, "admin.arena.setmap_not_found", mapName));
            return;
        }
        arena.setMapName(mapName);
        this.arenaManager.save(arena);
        player.sendMessage(this.lang.text(NamedTextColor.GREEN, "admin.arena.setmap_success", mapName));
    }
}

...
public class DeleteCommand extends BaseCommand implements SubCommand {
    ...
    public void execute(final CommandSender sender, final @NotNull String @NotNull [] args) {
        if (args.length < 2) {
            sender.sendMessage(this.lang.text(NamedTextColor.RED, "delete.usage"));
            return;
        }
        final String name = args[1];
        final Arena arena = this.arenaManager.get(name);
        if (arena == null) {
            sender.sendMessage(this.lang.text(NamedTextColor.RED, "delete.not_found", name));
            return;
        }
        this.arenaManager.delete(name);
        sender.sendMessage(this.lang.text(NamedTextColor.GREEN, "delete.success", name));
    }
}

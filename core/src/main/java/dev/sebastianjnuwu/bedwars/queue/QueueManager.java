...
public class QueueManager {
    ...
    public boolean addToQueue(@NotNull Player player, @NotNull String arenaName) {
        if (playerQueues.containsKey(player)) {
            CompatProvider.chat().sendMessage(player, this.lang.text(NamedTextColor.RED, "queue.already_in_queue"));
            return false;
        }

        queues.computeIfAbsent(arenaName, k -> new ConcurrentLinkedQueue<>()).offer(player);
        playerQueues.put(player, arenaName);
        CompatProvider.chat().sendMessage(player, this.lang.text(NamedTextColor.GREEN, "queue.joined", arenaName));
    ...
    public boolean removeFromQueue(@NotNull Player player) {
        ...
        CompatProvider.chat().sendMessage(player, this.lang.text(NamedTextColor.GREEN, "queue.left"));
        return true;
    }
    ...
    public void clearQueue(@NotNull String arenaName) {
        ...
        if (queue != null) {
            for (final Player player : queue) {
                ...
                CompatProvider.chat().sendMessage(player, this.lang.text(NamedTextColor.YELLOW, "queue.cleared", arenaName));
            }
        }
    }

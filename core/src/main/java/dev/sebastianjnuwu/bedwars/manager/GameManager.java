...
public class GameManager implements dev.sebastianjnuwu.bedwars.api.GameManager {
    ...
    private void enqueueJoin(final Player player, final String arenaName, final @Nullable String teamName, final @Nullable ArenaMode mode, final boolean teleport) {
        ...
        if (this.buildingArenas.contains(key)) {
            CompatProvider.chat().sendMessage(player, this.lang.text(NamedTextColor.YELLOW, "game.countdown_preparing"));
            return;
        }
        ...
        if (instance == null) {
            if (waiters != null) {
                for (final PendingJoin pending : waiters) {
                    ...
                    if (player != null) {
                        CompatProvider.chat().sendMessage(player, this.lang.text(NamedTextColor.RED, "game.world_not_ready", arenaName));
                    }
                }
            }
            return;
        ...
        if (!missing.isEmpty()) {
            ...
            if (waiters != null) {
                for (final PendingJoin pending : waiters) {
                    ...
                    CompatProvider.chat().sendMessage(player, this.lang.text(NamedTextColor.RED, "game.not_ready", arenaName));
                    for (final String msg : missing) {
                        CompatProvider.chat().sendMessage(player, this.lang.text(NamedTextColor.GRAY, "game.missing_entry", msg));
                    }
                }
            }
            return;

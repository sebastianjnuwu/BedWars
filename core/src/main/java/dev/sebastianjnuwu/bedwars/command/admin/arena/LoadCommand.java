...
public class LoadCommand extends BaseCommand implements SubCommand {
    ...
    public void execute(final CommandSender sender, final @NotNull String @NotNull [] args) {
        if (args.length < 2) {
            CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.RED, "load.usage"));
            return;
        }
        if (!(sender instanceof final Player player)) {
            CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.RED, "load.only_player"));
            return;
        }
        final String name = args[1];
        final Arena arena = this.arenaManager.get(name);
        if (arena == null) {
            CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.RED, "load.not_found", name));
            return;
        }
        ...
        if (Bukkit.getWorld(worldName) != null) {
            if (this.editorManager.isBeingEdited(canonical)) {
                CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.RED, "load.already_being_edited",
                        this.editorManager.getEditorName(canonical)));
                return;
            }
            CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.RED, "load.already_loaded", canonical));
            return;
        }

        final File file = this.arenaManager.getMapFile(arena);
        if (file == null) {
            CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.RED, "load.file_not_found"));
            return;
        }
        ...
        try {
            ...
            if (world == null) {
                CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.RED, "load.error",
            ...
            CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.GREEN, "load.success"));
        } catch (final Exception e) {
            CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.RED, "load.error", e.getMessage()));
        }
    }
}

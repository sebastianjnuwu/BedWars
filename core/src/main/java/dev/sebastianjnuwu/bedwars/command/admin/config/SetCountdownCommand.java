...
public class SetCountdownCommand extends BaseCommand implements ArenaSubCommand {
    ...
    public void execute(final CommandSender sender, final @NotNull Arena arena, final @NotNull String @NotNull [] args) {
        ...
        if (args.length < 4) {
            CompatProvider.chat().sendMessage(player, this.lang.text(NamedTextColor.RED, "admin.arena.setcountdown_usage"));
            return;
        }
        try {
            final int seconds = Integer.parseInt(args[3]);
            if (seconds < 3) {
                CompatProvider.chat().sendMessage(player, this.lang.text(NamedTextColor.RED, "admin.arena.setcountdown_invalid"));
                return;
            }
            arena.setCountdown(seconds);
            this.arenaManager.save(arena);
            CompatProvider.chat().sendMessage(player, this.lang.text(NamedTextColor.GREEN, "admin.arena.setcountdown_success", String.valueOf(seconds)));

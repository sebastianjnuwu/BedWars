...
public class ListCommand extends BaseCommand implements SubCommand {
    ...
    public void execute(final CommandSender sender, final @NotNull String @NotNull [] args) {
        ...
        if (names.isEmpty()) {
            CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.YELLOW, "list.empty"));
            return;
        }
        CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.GOLD, "list.header", String.valueOf(names.size())));
        for (final String name : names) {
            ...
            CompatProvider.chat().sendMessage(sender, this.lang.text("list.entry", name, status));
        }
    }
}

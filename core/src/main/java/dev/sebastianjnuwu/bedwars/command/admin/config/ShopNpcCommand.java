...
public class ShopNpcCommand extends BaseCommand implements ArenaSubCommand {
    ...
    public void execute(final CommandSender sender, final @NotNull Arena arena, final @NotNull String @NotNull [] args) {
        ...
        if (args.length < 4) {
            CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.RED, "admin.arena.shop_npc.usage_add"));
            CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.RED, "admin.arena.shop_npc.usage_remove"));
            CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.RED, "admin.arena.shop_npc.usage_list"));
            CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.RED, "admin.arena.shop_npc.usage_display_name"));
            return;
        }
        ...
        switch (action) {
            case "add" -> {
                ...
                if (npc != null) {
                    CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.GREEN, "admin.arena.shop_npc.added", arena.getName()));
                } else {
                    CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.GREEN, "admin.arena.shop_npc.added_no_npcs"));
                }
            }
            case "remove" -> {
                final List<ShopNpc> npcs = new ArrayList<>(arena.getShopNpcs());
                if (args.length > 4) {
                    try {
                        ...
                        if (id >= 0 && id < npcs.size()) {
                            ...
                            CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.GREEN, "admin.arena.shop_npc.removed", id));
                        } else {
                            CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.RED, "admin.arena.shop_npc.invalid_id"));
                        }
                    ...
                    this.shopNpcManager.removeEditorNpcs(arena.getName());
                    this.shopNpcManager.spawnEditorNpcs(arena.getName(), arena.getShopNpcs());
                    CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.GREEN, "admin.arena.shop_npc.removed_last"));
                }
            }
            case "list" -> {
                final List<ShopNpc> npcs = arena.getShopNpcs();
                if (npcs.isEmpty()) {
                    CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.YELLOW, "admin.arena.shop_npc.none"));
                } else {
                    CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.GREEN, "admin.arena.shop_npc.header", arena.getName()));
                    for (int i = 0; i < npcs.size(); i++) {
                        final ShopNpc npc = npcs.get(i);
                        final Location l = npc.location();
                        CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.GRAY, "admin.arena.shop_npc.entry", i, l.getBlockX(), l.getBlockY(), l.getBlockZ()));
                        CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.GRAY, "admin.arena.shop_npc.entry_skin", npc.skin()));
                        CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.GRAY, "admin.arena.shop_npc.entry_display_name",
            ...
            case "displayname" -> {
                ...
                try {
                    ...
                    this.shopNpcManager.removeEditorNpcs(arena.getName());
                    this.shopNpcManager.spawnEditorNpcs(arena.getName(), arena.getShopNpcs());
                    CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.GREEN, "admin.arena.shop_npc.display_name_set", id, name));
                } catch (final NumberFormatException e) {
                    CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.RED, "admin.arena.shop_npc.invalid_id"));
                }
            }
            default -> CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.RED, "admin.arena.shop_npc.unknown_action"));
        }
    }
}

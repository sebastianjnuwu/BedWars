...
public class SetSpawnCommand extends BaseCommand implements ArenaSubCommand {
    ...
    public void execute(final CommandSender sender, final @NotNull Arena arena, final @NotNull String @NotNull [] args) {
        ...
        if (args.length < 4) {
            CompatProvider.chat().sendMessage(player, this.lang.text(NamedTextColor.RED, "admin.arena.setspawn_usage"));
            return;
        }
        final String colorName = args[3].toLowerCase();
        final ArenaTeam team = arena.getTeam(colorName);
        if (team == null) {
            CompatProvider.chat().sendMessage(player, this.lang.text(NamedTextColor.RED, "admin.arena.setspawn_notfound", colorName));
            return;
        }
        ...
        CompatProvider.chat().sendMessage(player, this.lang.text(NamedTextColor.GREEN, "admin.arena.setspawn_success", colorName));
    }

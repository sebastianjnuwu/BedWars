...
public class ArenaRouter extends BaseCommand implements SubCommand {
    ...
    public void execute(final CommandSender sender, final @NotNull String @NotNull [] args) {
        if (args.length < 3) {
            CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.RED, "admin.arena.usage"));
            return;
        }
        ...
        if (cmd == null) {
            CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.RED, "admin.arena.unknown", args[2]));
            return;
        }

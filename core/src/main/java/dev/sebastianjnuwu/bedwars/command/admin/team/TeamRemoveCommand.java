...
public class TeamRemoveCommand extends BaseCommand implements ArenaSubCommand {
    ...
    public void execute(final CommandSender sender, final @NotNull Arena arena, final @NotNull String @NotNull [] args) {
        ...
        if (args.length < 4) {
            CompatProvider.chat().sendMessage(player, this.lang.text(NamedTextColor.RED, "admin.arena.removeteam_usage"));
            return;
        }
        ...
        if (team == null) {
            CompatProvider.chat().sendMessage(player, this.lang.text(NamedTextColor.RED, "admin.arena.removeteam_notfound", teamName));
            return;
        }
        ...
        CompatProvider.chat().sendMessage(player, this.lang.text(NamedTextColor.GREEN, "admin.arena.removeteam_success", teamName));
    }

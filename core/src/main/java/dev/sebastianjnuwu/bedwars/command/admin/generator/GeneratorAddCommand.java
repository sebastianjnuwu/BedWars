...
public class GeneratorAddCommand extends BaseCommand implements ArenaSubCommand {
    ...
    public void execute(final CommandSender sender, final @NotNull Arena arena, final @NotNull String @NotNull [] args) {
        ...
        if (args.length < 4) {
            CompatProvider.chat().sendMessage(player, this.lang.text(NamedTextColor.RED, "admin.arena.addgen_usage"));
            return;
        }
        ...
        if (!List.of("iron", "gold", "diamond", "emerald", "forge").contains(type)) {
            CompatProvider.chat().sendMessage(player, this.lang.text(NamedTextColor.RED, "admin.arena.addgen_invalid"));
            return;
        }
        ...
        if (type.equals("forge")) {
            if (args.length < 5) {
                CompatProvider.chat().sendMessage(player, this.lang.text(NamedTextColor.RED, "admin.arena.addgen_forge_usage"));
                return;
            }
            teamName = args[4].toLowerCase();
            final ArenaTeam team = arena.getTeam(teamName);
            if (team == null) {
                CompatProvider.chat().sendMessage(player, this.lang.text(NamedTextColor.RED, "admin.arena.addgen_forge_team_notfound", teamName));
                return;
            }
        }
        ...
        CompatProvider.chat().sendMessage(player, this.lang.text(NamedTextColor.GREEN, "admin.arena.addgen_success", type));
    }
}

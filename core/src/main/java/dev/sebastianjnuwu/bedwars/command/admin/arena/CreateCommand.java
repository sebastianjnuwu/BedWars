...
public class CreateCommand extends BaseCommand implements SubCommand {
    ...
    public void execute(final CommandSender sender, final @NotNull String @NotNull [] args) {
        ...
        if (args.length < 2) {
            CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.RED, "create.usage"));
            return;
        }
        ...
        if (this.arenaManager.get(name) != null) {
            CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.RED, "create.already_exists", name));
            return;
        }

        // Verifica se o template já existe
        if (this.templateManager.templateExists(name)) {
            CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.RED, "create.template_exists", name));
            return;
        }
        ...
        if (worldName == null) {
            CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.RED, "create.error", "Falha ao criar mundo de edição"));
            return;
        }
        ...
        CompatProvider.chat().sendMessage(sender, this.lang.text(
                NamedTextColor.GREEN,
                "create.success_void",
                name,
                worldName
        ));
        CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.YELLOW, "create.instructions"));

        if (!this.configManager.hasLobby()) {
            CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.YELLOW, "no_lobby"));
        }
    }

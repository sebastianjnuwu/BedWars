...
public class StartCommand extends BaseCommand {
    ...
    public void execute(final CommandSender sender, final @NotNull String @NotNull [] args) {
        ...
        if (args.length < 2) {
            CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.RED, "game.start_usage"));
            return;
        }
        final String arenaName = args[1];
        final Arena arena = this.arenaManager.get(arenaName);
        if (arena == null) {
            CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.RED, "game.arena_not_found", arenaName));
            return;
        }
        if (this.arenaManager.ensureWorldLoaded(arena) == null) {
            CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.RED, "game.world_not_ready", arenaName));
            return;
        }
        final Arena refreshed = this.arenaManager.get(arenaName);
        final List<String> missing = this.gameManager.validateArena(refreshed);
        if (!missing.isEmpty()) {
            CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.RED, "game.not_ready", arenaName));
            for (final String msg : missing) {
                CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.YELLOW, "game.missing_entry", msg));
            }
            return;
        }
        ...
        if (current != null) {
            if (current.getArena().getName().equalsIgnoreCase(arenaName)) {
                ...
                CompatProvider.chat().sendMessage(sender, this.lang.text(NamedTextColor.RED, "game.already_in_game"));
            }
            return;
        }
